#### HEADER ####
### Re-performing all analysis pertinent to competitive interactions for AmNat MS
### PTH June 2016


## OBSOLETE AFTER THIS LINE!
cres1 <- c.calc(c1)
...
# collate with life-history data:
comp2 <- read.table("comp2_final.txt",T,"\t",row.names=1)

# compare estimates from both approaches:
plot(comp2$c.t~res1$cT)

# generate master data.frame for plotting:
res2 <- cbind(res1,comp2[,c(1:3)])
res3 <- res2[,c(18:20,4:6,13:15)]
...
res3[,'na1'] <- 0
res3[,'na2'] <- 0
res3 <- res3[c(1,2,3,10,4,5,6,11,7,8,9)]

heatmap.2(as.matrix(res3),dendrogram=c("none"),trace=c("none"),
          density.info=c("none"),Rowv=FALSE,Colv=FALSE,
          na.color="gray",col=colorRampPalette(c("midnightblue","white","darkorange2")),
          notecex=0.6,notecol="black",symbreaks=TRUE,cexRow=0.6,cexCol=0.6,scale="column",colsep=c(1:9),rowsep=c(1:40),
          sepcolor="white",
...
# need to move labels to top and left, and to insert blank column after cols 3 and 6.
